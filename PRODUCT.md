...
- An AI chatbot (OpenRouter-backed) is a proof-of-skill demo, not just a convenience widget, when enabled : it should read as a working example of the RAG/agent skills claimed elsewhere on the site, not a bolted-on utility. **Currently disabled** (commented out in `app/layout.tsx`, not deleted) pending a reliable free-tier model — re-enable by uncommenting the `<Chatbot />` render once that's resolved.
...
- Chatbot backend uses OpenRouter, model `google/gemma-4-31b-it:free` (set via `OPENROUTER_MODEL`, same value as the code-level fallback if the env var is ever unset) — deliberate choice to stay on the free tier only, no paid fallback model wired in. Known tradeoff: this model intermittently 429s under OpenRouter's shared free-pool rate limit (observed roughly 1-in-3 failures in testing as of Sep 2026); accepted as the cost of $0/message. `thinkingmachines/inkling:free` (hard 403, agentic-harness clients only) and `poolside/laguna-s-2.1:free` (fully saturated, consistent 429) were tried and rejected as worse options. `moonshotai/kimi-k2` (paid, ~$0.00003/exchange, reliable in every test) remains the known-good option if reliability is ever prioritized over $0 cost. The system prompt was also compressed (dropped a redundant, stale README.md dump) and output is capped at `maxTokens: 500` to avoid a prior "insufficient credits for max possible tokens" failure mode. The Vercel AI SDK's default `toDataStreamResponse()` masks real provider errors as generic "An error occurred." — the route passes a `getErrorMessage` handler that logs and surfaces the actual error; keep that handler when touching this file.
...
- Security headers (CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy) are set in `next.config.js`. The CSP currently uses `'unsafe-inline'` for script-src and style-src as a pragmatic first pass (allowlisting exact origins for the SociableKit LinkedIn widget and Vercel Analytics without it risked breaking the widget, and this wasn't verified in a real browser this session) — tightening to a nonce-based CSP is a reasonable follow-up once someone can test it visually.
- An IndexNow key file exists at `public/d749f2d20b96d5b417f67c1e685e8999.txt` for Bing/Yandex verification, but no automated submission (POST to api.indexnow.org on publish) is wired up yet — that's a separate step.
- The Certifications section now shows only the IBM watsonx / Terraform / Credly badge credentials; the entry-level Coursera/Kaggle "Intro to X" courses were deliberately removed (confirmed by Aditya) as undercutting the senior "AI Architect" positioning.
- The hero has a text-labeled "Get in Touch" CTA (linking to `#contact`) and an explicit "open to full-time roles and AI consulting engagements" line — confirmed with Aditya that he is genuinely open to side consulting alongside the full-time Aintropy role, so this is not an invented claim.
